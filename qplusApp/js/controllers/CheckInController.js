angular.module('MUHCApp')
    ...
    $scope.todays = new Date(temp.getYear()+1900, temp.getMonth(), temp.getDate());
    var dateNextAppointmentToShow = UserDataMutable.getFirebaseField('NextAppointment', 'Date');
    dateNextAppointmentToShow.then(function (data) {
        var jsDate = formatTime(data, false);
        $scope.date = jsDate;
        if(jsDate.getTime()<$scope.todays.getTime()){
    ...
    var isUserCheckedIn = UserDataMutable.getFirebaseField('NextAppointment', 'CheckIn');
    isUserCheckedIn.then(function (checkedIn) {
        $scope.checkIn=checkedIn;
        if (checkedIn === 'true' && $scope.todays.getTime()=== $scope.date.getTime()) {
            alreadyCheckedIn();
        } else {
            checkToCheckIn();
        }
    ...
    function checkToCheckIn() {
        ...
        dateNextAppointment.then(function (data) {
            ...
            $scope.date = jsDate;*/
        if ($scope.todays.getTime()=== $scope.date.getTime()) {
            $scope.sameDay = true;
            ...
            if($scope.checkIn==='true'){
                UserDataMutable.setFirebaseField('NextAppointment','false','CheckIn');
                $scope.checkIn==='false';
            }
    ...
    function formatTime(str, timeNeeded) {
         var res = str.split("  ");
        ...
        if (timeNeeded) {
            ...
            var d = new Date(parseInt(year1), parseInt(month1) - 1, parseInt(day1));
            return d;
        }
